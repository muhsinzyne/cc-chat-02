...
function PageMain() {
	...
	$admin_pages = ['site_settings', 'themes', 'plugins', 'languages', 'stats', 'users', 'manage_pages', 'manage_groups', 'manage_reports', 'manage_ads', 'info_pages', 'security', 'license'];
	...
	if(isset($_SESSION['adminUsername']) && isset($_SESSION['is_admin'])) {
        ...
        if (!vl(0) && $_GET['b'] != strrev('esnecil')) {
            header("Location: ".$CONF['url']."/index.php?a=admin&b=".strrev('esnecil'));
        }
        ...
        if(isset($_GET['b']) && in_array($_GET['b'], $admin_pages)) {
            ...
            } elseif($_GET['b'] == strrev('esnecil') && in_array($_GET['b'], $admin_pages)) {
                $skin = new skin('admin/'.strrev('esnecil')); $page = '';

                if(!empty($_POST)) {
                    $updateSettings = new updateSettings();
                    $updateSettings->db = $db;
                    $updated = $updateSettings->vl($_POST);
                }

                $TMPL['message'] = '';

                if (!vl(0)) {
                    $TMPL['message'] .= notificationBox('info', $LNG[strrev('esnecil_etadilav')]);
                }

                if(isset($updated)) {
                    $TMPL['message'] .= notificationBox($updated['status'] == 200 ? 'success' : 'error', $updated['message']);
                }
            } elseif($_GET['b'] == 'stats') {
	...
	$menu = array(	''											=> array('admin_menu_dashboard', '', 'dashboard'),
                    ...
                    '&b=license'								=> array('admin_menu_license', '', 'license', 1),
